...
Hubc是服务器用来管理客户端的结构体，主要使用Guests保存客户端信息(未登录用户)，connection用来记录连接客户端信息，并打印日志，disconnect用来关闭客户端连接
以及结构体内的管道（管道不关闭，哪些依赖管道的协程就会一直跑下去，导致goroutine堵死，造成资源浪费或者程序卡死），统计登陆用户和访客的数量，管理
发布订阅中枢。
...
还增加了服务器的主动推送能力。前后端的消息传递统一使用json格式，方便解析易于开发，可读性好。框架定义了user和client结构体，明确了一个用户的不同
ws连接，使用hubc管理users，再使用user管理clients，层次分明，逻辑清晰。同时定义了PubSub、topic结构体，明确用户定义了哪些topic，使用PubSub
对用户进行消息的发送。